...
##>>>>>>>> Electricity <<<<###
datam <- read.table(file ="/media/helle/Data/Pinrolinvic/Drive E/Latihan R/ClairCityFinal/Gridded_master.csv", header=T, sep="," )
attach(datam)
dataP <- read.table(file ="/media/helle/Data/Pinrolinvic/Drive E/Latihan R/ClairCityFinal/SingleNOXPollutionSD.csv", header=T, sep="," )#
#attach(dataP)
DP <- dataP[c(-1)]
...
DP3 <- DP[3,]
DP4 <- DP[4,]
Z1 <- c(DP1,DP2,DP3,DP4)
Z2 <- data.frame(Z1)
Z3 <- as.numeric(Z2)
## Finding 2015 pattern1
Pattern <- Z3
upto <- length(datam$hh_per_ha)
PZPatt <- matrix(0,upto,96)
for (i in 1:upto){
  PZPatt[i] <- datam$hh_per_ha[i]*Pattern[1:96]
}
PatternE<- cbind(datam,PZPatt)
